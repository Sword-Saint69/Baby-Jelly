...
Run `npm run build:bear`, `build:cat`, or `build:bunny` to regenerate a
character solid (`gummy-bear`, `jelly-cat`, `jelly-bunny` bin/json, each with
its own cage, optical proxy, and thickness maps). All three reuse the reference
file's helpers and marching-tetrahedra polygonizer with per-character metaball
SDFs. `npm run test:characters` checks settling, volume, floor contact,
designed face organs landing on each sculpt, and muzzle and crown anchors.
...
| `npm run test:characters` | Verify bear/cat/bunny settling, designed organs, and anchors. |
