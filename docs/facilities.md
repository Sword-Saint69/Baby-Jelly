...
[`src/graphics/props.ts`](../src/graphics/props.ts) holds the tabletop scatter
and [`src/game/static-props.ts`](../src/game/static-props.ts) wraps it as a
non-interactive `Facility` (`active` is always false, `interactionDistance` is
always infinity, so it never steals the prompt). Tier A decor — pebbles,
crayons, leaves, marbles, pencil, buttons — is visual only. Tier B pieces —
wooden blocks, thread spools, die, dominoes, book stack, rubber duck, spinning
top — add oriented `CollisionBox`es resolved in `afterStep` behind the shared
`mayCollide` broad-phase gate, currently 11 boxes total.

Layout uses a seeded `mulberry32` scatter with exclusion disks around the four
playable facilities and spawn, so it is deterministic across loads and keeps
walk lanes and interaction radii clear. The shadow envelope is tightened to the
actual placements to protect ground-shadow resolution.

Two props are proximity-animated from fixed steps: the top spins up toward 14
rad/s with a slight lean while the body is within `.16` m and coasts back to an
exact rest pose, and the duck rocks while the body is within `.15` m. Poses snap
exactly to rest so idle shadow maps stay cached. Hard contact emits a short
`prop-knock` through the shared facility sound sink, throttled to one knock per
`.18` s for impacts above `.2` m/s. See `npm run test:props`.

## Prop interactions

The scatter is also a real interactive `Facility` with two hotspots sharing one
prompt slot. Within `.12` m grounded and ungrabbed, the nearer hotspot wins:

- **Ride Duck**: boards like the swing — cage nodes snap into a yawed seat
  frame stacked on the duck's back, then follow its rolling rock (`.16` rad peak
  over a 1.6 s period, ramping up over ~2.5 s) through graded stiffness and
  damping with frame-velocity feedforward, plus gravity compensation. Sustained
  rocking delights the baby after 3 s (`laughing`), the camera pulls to `.26`
  m, and the duck's own collision box is excluded from `afterStep` while riding
  (the remaining ten boxes still resolve). Dismount steps `.10` m to the +X side
  and rests the lowest point on the floor, retaining deformation; the proximity
  rock resumes from the ride's final angle so there is no visual snap.
- **Spin Top**: a momentary flick that overdrives the spin to ~22 rad/s with a
  knock click, without taking body ownership (`active` stays false, like the
  wearable table).

Boarding, riding, dismounting, and the periodic ride reminder all emit a
synthesized `duck-squeak` (a fast downward chirp with vibrato) through the same
sink, so mute, distance, and voice limits apply unchanged. Reset parks the
rocker and the top and clears the ride exactly.
