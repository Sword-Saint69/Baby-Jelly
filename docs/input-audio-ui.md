...
choices are lime, strawberry, blueberry, lemon, mint, grape, orange, and cola.
Each option supplies a surface color and three absorption coefficients. Choosing
one updates the body material and the optical field together, so surface tint
and transmitted light change as one design decision.
...
[`CharacterPicker`](../src/game/character-picker.ts) builds its options from
[`src/graphics/character-features.ts`](../src/graphics/character-features.ts):
Jelly Baby (classic), Jelly Cat (sleek sculpted climber, ω mouth, whiskers),
Gummy Bear (sculpted body with ears, muzzle, arms, and legs), and Jelly Bunny
(sculpted long-ears, buck tooth). Every character is its own sculpted solid
with its own cage, built by `npm run build:bear|cat|bunny` from per-character
metaball SDFs in [`scripts/build-character.mjs`](../scripts/build-character.mjs)
and loaded as `gummy-bear`, `jelly-cat`, or `jelly-bunny` bin/json. Small
attachments (cat ear tips, bear muzzle/nose) are skin-attached volumes sampled
through `FaceSkin` every frame, so they ride grabs, jumps, and facilities
without extra physics. Jelly-material shells follow flavor changes; inner-ear,
snout, nose, tooth, and whisker accents use small dedicated materials. Each
character carries a designed face in `CHARACTER_FACE_LAYOUT` (eye/brow/blush
placement and size, mouth shape and scale, optional buck tooth and rigid
whisker feelers) plus a resting lift and head-width fit. The choice persists in
`localStorage` and switching silhouettes reloads into the rebuilt body. The
picker mirrors the flavor menu interaction and is disposed with the runtime.
