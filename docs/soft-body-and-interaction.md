...
cooldown. Pressing jump once more mid-air spends one `.30` m/s bonus hop per
airborne trip (`.30` s cooldown, refreshed on the next grounded landing), which
also drives the wearable accessory hop. The resulting flight and landing are
still solved by the soft body.
