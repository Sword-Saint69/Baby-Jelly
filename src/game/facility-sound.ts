export type FacilitySoundKind='swing-creak'|'swing-air'|'trampoline-land'|'trampoline-spring'|'prop-knock'|'duck-squeak'|'slide-whoosh';
...
export function makeFacilitySample(kind:FacilitySoundKind,variant:number,sampleRate:number) {
  const duration=kind==='swing-creak'?.34:kind==='swing-air'?.32:kind==='trampoline-land'?.26:kind==='prop-knock'?.18:kind==='duck-squeak'?.22:kind==='slide-whoosh'?.3:.38;
  ...
  const modes=kind==='trampoline-land'?[79,127,193]:kind==='trampoline-spring'?[237,391,617]:kind==='prop-knock'?[211,347,523]:[183,307,479];
  ...
  for(let i=0;i<samples.length;i++) {
    ...
    let value=0;
    if(kind==='swing-air'||kind==='slide-whoosh')value=(low-slow)*Math.sin(Math.PI*t/duration)**2*.36;
    else if(kind==='prop-knock') {
      // A short wooden knock: click transient plus fast-decaying hollow partials.
      value=(low-slow)*Math.exp(-t/.012)*.5;
      for(let j=0;j<modes.length;j++) {
        value+=Math.sin(2*Math.PI*modes[j]*detune*t)*Math.exp(-t/(.030/(1+j*.4)))*.10/(j+1);
      }
      value*=attack;
    } else if(kind==='duck-squeak') {
      // A rubber-duck squeak: a fast downward chirp with vibrato and breath.
      squeakPhase+=2*Math.PI*(720*detune-290*detune*t/duration+40*Math.sin(2*Math.PI*30*t))/sampleRate;
      value=Math.sin(squeakPhase)*Math.exp(-t/.085)*.5+(low-slow)*Math.exp(-t/.03)*.06;
      value*=attack;
...
export class FacilityAudio {
  ...
  private readonly customSamples=new Map<FacilitySoundKind,AudioBuffer>();
  ...
  /** Override the synthesized texture for a kind with an external recording (e.g. collide.mp3). */
  setCustomSample(kind:FacilitySoundKind,buffer:AudioBuffer) {this.customSamples.set(kind,buffer);}
  hasCustomSample(kind:FacilitySoundKind) {return this.customSamples.has(kind);}
  play(event:FacilitySoundEvent,distance:number,pan:number) {
    ...
    let buffer=this.customSamples.get(event.kind)??this.buffers.get(key);
    if(!buffer) {
    ...
    const level=event.kind==='trampoline-land'?.38:event.kind==='swing-creak'?.28:event.kind==='prop-knock'?.30:event.kind==='duck-squeak'?.32:event.kind==='slide-whoosh'?.26:.22;
  ...
  dispose() {this.stop();this.buffers.clear();this.customSamples.clear();}
}
