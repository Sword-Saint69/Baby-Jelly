...
export class Input {
  ...
  private begin=(e:PointerEvent)=>{
    ...
    if(e.pointerType==='mouse')void this.sound.startDrag().catch(()=>{});
  };
  ...
  private syncDragSound() {
    const mouseHeld=[...this.grabs.values()].some(state=>state.pointerType==='mouse'&&!state.releasePending);
    if(!mouseHeld)this.sound.stopDrag();
  }
  private finishRelease=(id?:number)=>{
    ...
    this.body.wake();this.syncGrabControls();this.syncDragSound();
  };
