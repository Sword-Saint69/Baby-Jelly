...
export class Locomotion {
  ...
  /** One bonus hop per airborne trip, spent until the next grounded landing. */
  private airJumps=0;
  ...
  reset() { this.yaw=0; this.phase=0;this.gaitWeight=0;this.jumpCooldown=0; this.jumpQueued=false; this.airJumps=0; this.move.set(0,0,0); }
  step(h:number) {
    ...
    if(this.jumpQueued&&this.jumpCooldown<=0) {
      if(this.grounded) {
        for(let i=0;i<b.mass.length;i++) {
        ...
        this.airJumps=1;this.onJump();
        this.jumpCooldown=.24;
      } else if(this.airJumps>0) {
        // A smaller mid-air hop: same foot-weighted shape, about 70% impulse.
        for(let i=0;i<b.mass.length;i++) {
          const foot=Math.max(0,1-b.rest[i*3+1]/.035);
          v[i*3+1]+=.30+foot*.085;
        }
        this.airJumps--;this.onJump();
