...
import { CharacterPicker, getStoredCharacter } from './character-picker.ts';
...
import { SlideFacility } from './slide-facility.ts';
import { SeesawFacility } from './seesaw-facility.ts';
...
export async function startGame(stage:(s:string)=>void,fail:(e:unknown)=>void) {
  ...
  const initialCharacter=getStoredCharacter();
  const body=new SoftBody(await loadBabyCage(initialCharacter));
  const baby=new Baby(body,initialCharacter);scene.add(baby.group);
  ...
  facilities.add(new SlideFacility(scene,body,facilityShadows,sound.facility));
  facilities.add(new SeesawFacility(scene,body,facilityShadows,sound.facility));
  facilities.add(bed);
  facilities.add(new StaticProps(scene,body,facilityShadows,sound.facility));
  const flavorPicker=new FlavorPicker(flavor=>{
    baby.setFlavor(flavor);optics.setAbsorption(JELLY_FLAVORS[flavor].absorption);
  });
  const characterPicker=new CharacterPicker(character=>{
  ...
  const dispose=()=>{
    ...
    facilities.dispose();facilityShadows.dispose();flavorPicker.dispose();characterPicker.dispose();composite.dispose();baby.dispose();table.dispose();environment.dispose();optics.dispose();renderer.dispose();
  };
  window.addEventListener('pagehide',event=>{if(!event.persisted)dispose();});
  if(import.meta.hot)import.meta.hot.dispose(dispose);
  return {stop:()=>{disposed=true;lightingMode.dispose();input.clear();facilities.dispose();facilityShadows.dispose();flavorPicker.dispose();characterPicker.dispose();sound.dispose();transport.dispose();void renderer.setAnimationLoop(null);}};
}
