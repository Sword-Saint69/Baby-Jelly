...
export class JellySound {
  ...
  private dragLoad:Promise<AudioBuffer|null>|null=null;
  private dragSource:AudioBufferSourceNode|null=null;
  private dragGain:GainNode|null=null;
  private collideLoad:Promise<void>|null=null;
  private collideReady=false;
  ...
  constructor() {
    ...
    document.addEventListener('visibilitychange',()=>{if(document.hidden){this.stopFacilities();this.stopDrag();}},{signal});
  }
  ...
  unlock() {
    ...
    // Fetch + decode early so the first prop thump already uses the recording.
    // decodeAudioData works while suspended; play() still waits for running.
    void this.ensureCollideSample(context).catch(()=>{});
  ...
  /** Load collide.mp3 once and use it for prop-collision thumps. Falls back to synth on failure. */
  private ensureCollideSample(context:AudioContext) {
    if(this.collideReady||this.facilities?.hasCustomSample('prop-knock')){this.collideReady=true;return Promise.resolve();}
    if(this.collideLoad)return this.collideLoad;
    this.collideLoad=fetch(`${import.meta.env.BASE_URL}collide.mp3`)
      .then(response=>{if(!response.ok)throw new Error(`collide sound ${response.status}`);return response.arrayBuffer();})
      .then(data=>context.decodeAudioData(data))
      .then(buffer=>{this.facilities?.setCustomSample('prop-knock',buffer);this.collideReady=true;})
      .catch(()=>{})
      .finally(()=>{this.collideLoad=null;});
    return this.collideLoad;
  }
  private loadDragBuffer(context:AudioContext) {
    if(this.dragBuffer)return Promise.resolve(this.dragBuffer);
    if(this.dragLoad)return this.dragLoad;
    this.dragLoad=fetch(`${import.meta.env.BASE_URL}drag.mp3`)
      .then(response=>{if(!response.ok)throw new Error(`drag sound ${response.status}`);return response.arrayBuffer();})
      .then(data=>context.decodeAudioData(data))
      .then(buffer=>{this.dragBuffer=buffer;return buffer;})
      .catch(()=>null)
      .finally(()=>{this.dragLoad=null;});
    return this.dragLoad;
  }
  /** Loop the custom drag sound while a mouse grab is held. Silent when muted via master gain. */
  async startDrag() {
    if(document.hidden||this.dragSource)return;
    await this.unlock().catch(()=>{});
    const context=this.context, out=this.master;
    if(!context||!out||context.state!=='running')return;
    const buffer=await this.loadDragBuffer(context);
    if(!buffer||this.dragSource||document.hidden||this.context!==context)return;
    const source=context.createBufferSource(),gain=context.createGain();
    source.buffer=buffer;source.loop=true;gain.gain.value=.5;
    source.connect(gain).connect(out);
    source.onended=()=>{
      if(this.dragSource===source){this.dragSource=null;this.dragGain=null;}
      source.disconnect();gain.disconnect();
    };
    this.dragSource=source;this.dragGain=gain;
    source.start();
  }
  stopDrag() {
    const source=this.dragSource,gain=this.dragGain;
    this.dragSource=null;this.dragGain=null;
  ...
  dispose() {
    this.stopDrag();this.collideLoad=null;this.collideReady=false;this.facilities?.dispose();this.facilities=null;
