...
const CHARACTER_ASSETS:Record<string,[string,string]>={
  jelly:['jelly-baby.bin','jelly-baby.json'],
  cat:['jelly-cat.bin','jelly-cat.json'],
  bear:['gummy-bear.bin','gummy-bear.json'],
  bunny:['jelly-bunny.bin','jelly-bunny.json'],
};

export async function loadBabyCage(character='jelly') {
  // Every character is its own sculpted solid with its own cage; the loader
  // just picks the asset pair.
  const assets=CHARACTER_ASSETS[character]??CHARACTER_ASSETS.jelly;
  const [binary,metadata]=await Promise.all([
    fetch(new URL(`../assets/model/${assets[0]}`,import.meta.url)),
    fetch(new URL(`../assets/model/${assets[1]}`,import.meta.url)),
  ]);
